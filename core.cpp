			...
			try
			{
				...
				if (!CLIENT->Connected() || CLIENT->Errors())
				{
					...
					if (CLIENT != nullptr)
					{
						...
						{
							printf("Cannot connect to server...Exiting\n\n");
							nReconnectCount++;
							exit(0);
						}
				...
				if (TIMER.Elapsed() > nTimerWait)
				{
					...
					printf("[Stats] %.03f MH/s | Blks/24h %.03f | A=%u R=%u | Diff=%.04f | %02d:%02d:%02d:%02d\n", 
						MegaHashesPerSecond, (double)nBlocksFoundCounter / (double)elapsedFromStart * 86400.0, nBlocksAccepted, nBlocksRejected, diff, t.tm_yday, t.tm_hour, t.tm_min, t.tm_sec);
